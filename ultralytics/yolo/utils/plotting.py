...
@plt_settings()
def plot_results(file='path/to/results.csv', dir='', segment=False, pose=False, classify=False, on_plot=None, labels_list=None):
    """Plot training results.csv. Usage: from utils.plots import *; plot_results('path/to/results.csv')."""
    import pandas as pd
    save_dir = Path(file).parent if file else Path(dir)

    if labels_list: # edited (Added)
        for name, keep in [[labels_list[0] ,'first'], [labels_list[1], 'last']]: 
            if classify:
            ...
            for f in files:
                try:
                    data = pd.read_csv(f).drop_duplicates(subset=['                  epoch'], keep=keep)
            ...
            ax[1].legend()
            fname = save_dir / f'results_{name}.png'
