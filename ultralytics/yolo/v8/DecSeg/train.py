...
class DetectionSegmentationTrainer(BaseTrainer):
    ...
    def plot_metrics(self, labels_list): # edited
        """Plots metrics from a CSV file."""
        plot_results(file=self.csv, on_plot=self.on_plot, labels_list=labels_list)  # save results.png
...
class Loss:
    ...
    def __call__(self, preds, batch):
        ...
        # edited
        # restart the class mapping at 0 if batch['cls'] is full of 5
        if torch.max(batch['cls']) == torch.tensor(5.0):
            batch['cls'] = torch.full((batch['cls'].shape[0], batch['cls'].shape[1]), 0, dtype=torch.float32)

        # edited
        # set the batch['cls'] on the same device as others
        if not (batch['batch_idx'].is_cuda == batch['cls'].is_cuda == batch['bboxes'].is_cuda):
            batch['cls'] = batch['cls'].to("cuda:0" if (batch['batch_idx'].is_cuda and batch['bboxes'].is_cuda)  else "cpu")
