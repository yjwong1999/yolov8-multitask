                ...
                if self.training:
                    if self.args.task == 'multi':
                        for i, preds in enumerate(preds_list):
                            # edited
                            # restart the class mapping at 0 if batch[i]['cls'] is full of 5
                            if torch.max(batch[i]['cls']) == torch.tensor(5.0):
                                batch[i]['cls'] = torch.full((batch[i]['cls'].shape[0], batch[i]['cls'].shape[1]), 0, dtype=torch.float32)
            ...
            if self.args.task == 'multi':
                for i, label_name in enumerate(self.data['labels_list']):
                    if 'det' in label_name:
                        ...
                        if not (batch[i]['batch_idx'].is_cuda == batch[i]['cls'].is_cuda == batch[i]['bboxes'].is_cuda):
                            batch[i]['cls'] = batch[i]['cls'].to("cuda:0" if (batch[i]['batch_idx'].is_cuda and batch[i]['bboxes'].is_cuda) else "cpu")
